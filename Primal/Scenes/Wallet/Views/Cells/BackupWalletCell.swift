...
protocol BackupWalletCellDelegate: AnyObject {
    func backupWalletPressed()
}

final class BackupWalletCell: UITableViewCell, Themeable {
    let descLabel = UILabel()
    let actionButton = LargeRoundedButton(title: "Backup Wallet Now")
    
    weak var delegate: BackupWalletCellDelegate?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        actionButton.addAction(.init(handler: { [weak self] _ in
            self?.delegate?.backupWalletPressed()
        }), for: .touchUpInside)
    ...
    func updateTheme() {
        ...
        let descText = NSMutableAttributedString(string: "IMPORTANT: ", attributes: [
            .font: UIFont.appFont(withSize: 15, weight: .bold),
            .foregroundColor: UIColor.foreground5,
            .paragraphStyle: paragraphStyle,
            .underlineStyle: NSUnderlineStyle.single.rawValue
        ])
        descText.append(.init(string: "This wallet has not been\nbacked up. You need to backup your wallet\nso you don’t lose access to your funds.", attributes: [
