//
//  FloatingPlayerMoveGesture.swift
//  Primal
//
...
public final class FloatingPlayerMoveGesture: UIPanGestureRecognizer {
    ...
    static var edgePositions: [CGPoint] {
        ...
        let topLeft = CGPoint(x: margin + smallVideoWidth / 2, y: view.safeAreaInsets.top + topBarMargin)
        let topRight = CGPoint(x: view.frame.width - (margin + smallVideoWidth / 2), y: view.safeAreaInsets.top + topBarMargin)
    ...
    @objc func gesture() {
        ...
        case .changed:
            ...
            self.floatingView?.showChevron = view.center.x < -50 || view.center.x > rootView.frame.width + 50
        case .ended, .cancelled:
            ...
            UIView.animate(
                ...
                animations: {
                    view.center = target
                    self.floatingView?.showChevron = target.x < 0 || target.x > rootView.frame.width
                },
                completion: nil
            )
        default:
