...
struct PrimalWalletRequest {
  enum RequestType {
    ...
    case inAppPurchase(transactionId: String, quote: Decimal)
    case quote(productId: String)
    ...
    var requestContent: String {
      switch self {
        ...
        case .inAppPurchase(let transactionId, let quote):
          return "[\"in_app_purchase\", {\"return\": {\"transactionId\":\"\(transactionId)\", \"quote\": \(quote)}]"
        case .quote(let productId):
          return "[\"quote\", {\"productId\": \"\(productId)\"}]"
      }
    }
  }
...
private extension PrimalWalletRequest {
  ...
  func handleWalletEvent(_ payload: [String: JSON]) {
    ...
    switch kind {
      ...
      case .WALLET_QUOTE:
        guard
          let quote = Decimal(string: contentString)
        else {
          print("Unable to handle WALLET_QUOTE")
          return
        }
        pendingResult.quote = quote
      case .WALLET_IN_APP_PURCHASE:
        print("UNHANDLED KIND: \(kind)")
    }
  }
}
